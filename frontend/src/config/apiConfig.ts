...
function getApiBaseUrl(): string {
  ...
  // Проверяем сохраненные настройки сервера
  ...
  if (currentHost === 'localhost' || currentHost === '127.0.0.1') {
    // Локальная разработка - подключаемся к удаленному серверу
    // Замените на IP адрес вашего удаленного сервера
    return 'http://10.20.3.135:8000'; // IP адрес удаленного сервера
  } else if (currentPort === '3000' || currentPort === '') {
    // Разработка - фронтенд на 3000, бэкенд на удаленном сервере
    return 'http://10.20.3.135:8000'; // IP адрес удаленного сервера
